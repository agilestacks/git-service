    ...
    stage('Build') {
      steps {
        container('toolbox') {
          dir('api') {
            script {
              final component = hub.explain(state: params.HUB_STATE).components['git-service-ecr']
      ...
      steps {
        container('toolbox') {
          script {
            ...
            final name = 'git'
            sh script: "kubectl -n ${namespace} --record 'deployment/${name}' set image 'git-service=${image}'"
          }
        }
      }
    }
